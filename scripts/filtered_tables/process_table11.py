...
# === Ensure numeric types and convert percentages to proportions ===
percent_cols = ["Percent Women", "Percent White", "Percent Black", "Percent Asian", "Percent Hispanic"]
for col in percent_cols:
    df_11[col] = pd.to_numeric(df_11[col], errors="coerce") / 100

# === Clean occupation names for matching ===
df_11["Occupation_clean"] = df_11["Occupation"].str.lower().str.strip()

# === Extract SOC Code (if any) from occupation string ===
df_11["SOC_Code"] = df_11["Occupation"].str.extract(r"(\d{2}-\d{4})")
